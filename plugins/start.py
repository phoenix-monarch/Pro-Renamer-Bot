...
bot_username = os.environ.get("BOT_USERNAME","Mrkiller_Renamer_Bot")
log_channel = int(os.environ.get("LOG_CHANNEL", ""))
token = os.environ.get('TOKEN', '')
botid = token.split(':')[0]
FLOOD = 500
MRKILLER_PIC = os.environ.get("MRKILLER_PIC", "")
...
async def start(client, message):
	...
	I am file renamer bot, Please sent any telegram**Document Or Video** and enter new filename to rename it"""
        await message.reply_photo(photo=MRKILLER_PIC,
                                caption=txt,
                                reply_markup=InlineKeyboardMarkup(
                                      ...
                                      InlineKeyboardButton("Movie Channel", url='https://t.me/Movie_Flix_Zone')],
                                      [InlineKeyboardButton("☕ Buy Me A Coffee ☕", url='https://p.paytm.me/xCTH/t6g0h9l8')]
                                      ]))
        return
    if id:
        if old == True:
...
async def send_doc(client, message):
    ...
    if update_channel:
        try:
            ...
            await client.send_message(log_channel,f"🦋 #Mrkiller_LOGS 🦋,\n\n**ID** : `{user_id}`\n**Name**: {message.from_user.first_name} {message.from_user.last_name}\n**User-Plan** : {user}\n\n ",
    ...
    try:
        ...
        await message.reply_text(text=f"Hello dear {message.from_user.first_name}  **we are currently working on this issue**\n\nPlease try to rename files from your another account.\nBecause this BOT can't rename file sent by some ids.\n\nIf you are an **ADMIN** Don't worry ! here we have a solution for you dear {message.from_user.first_name }.\n\nPlease use \n👉 `/addpremium your_other_userid` 👈 to use premium feautres\n\n",
                                  reply_markup=InlineKeyboardMarkup([
                                                                     [InlineKeyboardButton("🦋 Contact Mrkiller Developer 🦋", url='https://telegram.me/Mrkiller_1109')],
                                                                     [InlineKeyboardButton("🔺 Watch Tutorial 🔺", url='https://youtube.com/@LazyDeveloperr')],
                                                                     [InlineKeyboardButton("🦋 Visit Channel  ", url='https://t.me/Hpbot_update'),
                                                                     InlineKeyboardButton("  Support Group 🦋", url='https://t.me/HP_Bot_discuss_group')],
                                                                     [InlineKeyboardButton("☕ Buy Me A Coffee ☕", url='https://p.paytm.me/xCTH/t6g0h9l8')]
                                                                    ]))
        await message.reply_text(text=f"🦋")
